...
const initialConfig = async (initialFsListState: string[]): Promise<ConfigStore> => {
  let initialState = null;
  let hasChanged = false;

  try {
    // check if file exists
    const configFileContent = await readFile('/.markee', {encoding: 'utf8'});
  ...
  if (initialFsListState.length) {
    const workspaceNames = initialFsListState.filter((item: string) => item.split('/').length === 2).map((item: string) => item.split('/')[1]);
    ...
    workspaceNames.forEach((workspaceName: string) => {
      if (!initialState.workspaces[`/${workspaceName}`]) {
        initialState.workspaces[`/${workspaceName}`] = structuredClone({
          ...workspaceTemplate, name: workspaceName
        });
        hasChanged = true;
      }
    });

    const collectionFolders = initialFsListState.filter((item: string) => item.split('/').length === 3);
    ...
    collectionFolders.forEach((collectionFolder: string) => {
      ...
      if (!initialState.collections[collectionFolder]) {
        initialState.collections[collectionFolder] = structuredClone({
          ...collectionTemplate, name: collectionName
        });
        hasChanged = true;
      }
    });

    const noteFiles = initialFsListState.filter((item: string) => item.split('/').length === 4);
  ...
  if (hasChanged) {
    try {
      await writeFile('/.markee', JSON.stringify(initialState, null, 2), { encoding: 'utf8', mode: 0o666 });
    } catch {}
  }
