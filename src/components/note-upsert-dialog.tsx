...
import {
    ...
    SelectTrigger,
    SelectValue
} from "@/editor/registry/new-york/ui/select.tsx";

export function NoteUpsertDialog({ setCollection, collection }: { setCollection: SetStateAction<ConfigStoreCollection | null>, collection: ConfigStoreCollection }) {
    const { activeWorkspace, createNote, collectionNotesCallback } = useMarkee();
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        await createNote(activeWorkspace.name, collection.name, values.title, values.type);
        setCollection(null);
        form.reset();
    }
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="grid grid-cols-4 items-center gap-4">
                                ...
                                <FormControl>
                                    <Input className="col-span-3" placeholder="Type in a title for your note" {...field} />
                                </FormControl>
                        ...
                        render={({ field }) => (
                            <FormItem className="grid grid-cols-4 items-center gap-4">
                                ...
                                <Select {...field}>
                                    ...
                                    <SelectContent>
                                        <SelectItem value="md">Markdown (recommended)</SelectItem>
